...
@warehouse_inventory_router.post(
    "/warehouse_inventory_move", response_model=WarehouseInventorySchema
)
async def move_warehouse_inventory(
    ...
    existing_warehouse_inventories: List[
        WarehouseInventory
    ] = existing_warehouse_inventories.scalars().all()
    ...
    if len(existing_warehouse_inventories) == 1:
        ...
        if (
            existing_warehouse_inventories[0].row == warehouse_inventory_move.source_row
            and existing_warehouse_inventories[0].column
            == warehouse_inventory_move.source_column
        ):
            source_inventory = existing_warehouse_inventories[0]
    ...
    # remove from source
    sku_variant_index = index_with_default(
        source_inventory.sku_variants, warehouse_inventory_move.sku_variant_id, None
    )
    if sku_variant_index is None:
        raise Exception(
            f"Cannot move missing sku_variant: {warehouse_inventory_move.sku_variant_id} from inventory"
        )
    ...
    if source_inventory.quantities[sku_variant_index] < 0:
        raise Exception(
            f"Cannot move quantity: {warehouse_inventory_move.quantity} from inventory"
        )

    source_inventory.projected_quantities[
        sku_variant_index
    ] -= warehouse_inventory_move.quantity
    ...
    if (
        source_inventory.projected_quantities[sku_variant_index] == 0
        and source_inventory.quantities[sku_variant_index] == 0
    ):
    ...
    if target_inventory is None:
        ...
        sku_variant_index = index_with_default(
            target_inventory.sku_variants, warehouse_inventory_move.sku_variant_id, None
        )
        if sku_variant_index is None:
            target_inventory.projected_quantities = (
                target_inventory.projected_quantities
                + [warehouse_inventory_move.quantity]
            )
            target_inventory.quantities = target_inventory.quantities + [
                warehouse_inventory_move.quantity
            ]
            target_inventory.sku_variants = target_inventory.sku_variants + [
                warehouse_inventory_move.sku_variant_id
            ]
            ...
            target_inventory.quantities[
                sku_variant_index
            ] += warehouse_inventory_move.quantity
            target_inventory.projected_quantities = list(
                target_inventory.projected_quantities
            )
...
@warehouse_inventory_router.post(
    "/warehouse_inventory", response_model=WarehouseInventorySchema
)
async def post_warehouse_inventory(
    ...
    if new_sku_variant:
        ...
        if warehouse_inventory.sku_variant_id is not None:
            print(
                "Warehouse Inventory SKU Variant should be None if we are creating a new SKU Variant"
            )
        new_sku_variant = SKUVariant(
            created_at=datetime.datetime.utcnow(),
            parent_sku_id=warehouse_inventory.sku_id,
            id=uuid4(),
        )
    ...
    existing_warehouse_inventory_result = await session.execute(
        select(WarehouseInventory).where(
            and_(
    ...
    existing_warehouse_inventory: Optional[
        WarehouseInventory
    ] = existing_warehouse_inventory_result.scalars().one_or_none()
    if existing_warehouse_inventory:
        sku_variant_index = index_with_default(
            existing_warehouse_inventory.sku_variants,
            warehouse_inventory.sku_variant_id,
        )
        if sku_variant_index is not None:
            ...
            if new_quantities[sku_variant_index] == 0:
                ...
                new_projected_quantities = list(
                    existing_warehouse_inventory.projected_quantities
                )
            ...
            print(
                "SKU Variant: ",
                warehouse_inventory.sku_variant_id,
                " has an incorrect projected quantity. Possibly unaccounted/missing from Purchase Orders",
            )
            new_projected_quantities = list(
                existing_warehouse_inventory.projected_quantities
            )
