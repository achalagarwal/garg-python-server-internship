...
async def delete_invoice(
    force_delete: bool, invoice_id: UUID4, session: AsyncSession = Depends(get_session)
):
    invoice_query = (
        select(Invoice)
        .options(joinedload(Invoice.warehouse_invoices))
    ...
    for warehouse_invoice in invoice.warehouse_invoices:
        ...
        if warehouse_invoice.status in {
            InvoiceStatus.PICKING,
            InvoiceStatus.READY_FOR_TRANSIT,
            InvoiceStatus.IN_TRANSIT,
        }:
    ...
    for warehouse_invoice in invoice.warehouse_invoices:
        ...
        for i, warehouse_inventory_id in enumerate(
            warehouse_invoice.warehouse_inventories
        ):
            ...
            if sku_variant is None:
                continue
            warehouse_inventory: WarehouseInventory = warehouse_inventory_id_map[
                warehouse_inventory_id
            ]
            index_in_warehouse_inventory = index_with_default(
                warehouse_inventory.sku_variants, sku_variant
            )
...
async def post_invoice(
    ...
    if len(invoice.items) == 0:
        raise HTTPException(
            status_code=422, detail=[{"loc": ["Items"], "msg": "missing"}]
        )
    ...
    sku_variants_query = (
        select(SKUVariant)
        .where(
            SKUVariant.parent_sku_id.in_(invoice.items),
            SKUVariant.id == any_(WarehouseInventory.sku_variants),
        )
        .order_by(SKUVariant.created_at)
    )
    ...
    sku_variant_to_parent_sku = {
        sku_variant.id: sku_variant.parent_sku_id for sku_variant in sku_variants
    }
    ...
    oldest_sku_variant_ids = set(
        sku_variants_list[0] for _, sku_variants_list in sku_variants_map.items()
    )

    # TODO: detect best warehouse_id depending on availability of items and destination address
    warehouse_inventories_query = select(WarehouseInventory).where(
        WarehouseInventory.sku_variants.op("&&")(oldest_sku_variant_ids)
    )
    ...
    sku_quantity_remaining_map = {
        sku_id: invoice.quantities[i] for i, sku_id in enumerate(invoice.items)
    }
    ...
    sku_quantity_remaining_map = dict(
        filter(lambda x: x[1] > 0, sku_quantity_remaining_map.items())
    )
    ...
    for sublist in [
        sku_variants_map[sku_id][1:] for sku_id in sku_quantity_remaining_map
    ]:
        next_sku_variant_ids.extend(sublist)

    # TODO: use best warehouse_id from above
    warehouse_inventories_query = (
        select(WarehouseInventory)
        .with_for_update()
        .where(WarehouseInventory.sku_variants.op("&&")(next_sku_variant_ids))
    )
    warehouse_inventories_result = await session.scalars(warehouse_inventories_query)
    warehouse_inventories: List[WarehouseInventory] = warehouse_inventories_result.all()

    # create a map from sku_variant_id to warehouse_inventories
    sku_variant_to_warehouse_inventories_and_index: List[
        Tuple[WarehouseInventory, int]
    ] = defaultdict(list)
    for warehouse_inventory in warehouse_inventories:
        for index, sku_variant_id in enumerate(warehouse_inventory.sku_variants):
            sku_variant_to_warehouse_inventories_and_index[sku_variant_id].append(
    ...
    for sku_id, quantity_remaining in sku_quantity_remaining_map.items():
        ...
        for sku_variant_id in sku_variant_ids:
            ...
            for (
                warehouse_inventory,
                index,
            ) in sku_variant_to_warehouse_inventories_and_index[sku_variant_id]:
                if quantity_remaining == 0:
                    break
                quantity_in_inventory = warehouse_inventory.projected_quantities[index]
                take_from_inventory = min(quantity_remaining, quantity_in_inventory)
    ...
    warehouse_inventory_picks.sort(
        key=lambda id_and_warehouse_inventory_pick: id_and_warehouse_inventory_pick[
            1
        ].row
    )
    row_boundaries = []
    ...
    sorted_warehouse_inventory_picks: List[
        Tuple[uuid.UUID, WarehouseInventoryPick]
    ] = []
    reversed = False
    for i, row_boundary in enumerate(row_boundaries):
        ...
        if next_row_boundary is not None:
            if (
                warehouse_inventory_picks[next_row_boundary][1].row
                != warehouse_inventory_picks[row_boundary][1].row + 1
            ):
                reversed = False
            else:
    ...
    for sku_id, quantity in sku_quantity_remaining_map.items():
        sorted_warehouse_inventory_picks.append(
            (
                None,
                WarehouseInventoryPick(
                    row=None,
    ...
    warehouse_invoice = WarehouseInvoice(
        ...
        warehouse_inventories=list(
            map(
                lambda id_and_warehouse_inventory_pick: id_and_warehouse_inventory_pick[
                    0
                ],
...
async def get_pending_invoices(
    page: int = 0, page_size: int = 25, session: AsyncSession = Depends(get_session)
):
    invoices_result = await session.execute(
        ...
        .where(
            or_(
                ...
                Invoice.status == InvoiceStatus.PICKING,
            )
        )
    ...
    for invoice in invoices:
        ...
        invoice_items_result = await session.execute(
            select(SKU).where(
                SKU.id.in_([str(id) for id in invoice.items]),
            )
        )
        invoice_result["items"] = [
            invoice_items.__dict__
            for invoice_items in invoice_items_result.scalars().all()
        ]
...
async def patch_invoice(
    ...
    warehouse_invoice = await session.execute(
        ...
        .options(
            joinedload(WarehouseInvoice.parent_invoice).options(
                joinedload(Invoice.warehouse_invoices)
            )
        )
...
async def get_pending_warehouse_invoices(
    ...
    page: int = 0,
    page_size: int = 50,
    last_updated_timestamp: Union[str, None] = None,
    session: AsyncSession = Depends(get_session),
):
    ...
    warehouse_invoices_query = (
        select(WarehouseInvoice)
        .options(joinedload(WarehouseInvoice.parent_invoice))
        .where(
            WarehouseInvoice.warehouse_id == warehouse_id,
            or_(
                WarehouseInvoice.status == InvoiceStatus.PENDING,
                WarehouseInvoice.status == InvoiceStatus.PICKING,
                WarehouseInvoice.status == InvoiceStatus.IN_TRANSIT,
                WarehouseInvoice.status == InvoiceStatus.READY_FOR_TRANSIT,
            ),
        )
        .order_by(WarehouseInvoice.created_at)
        .offset(page * page_size)
        .limit(page_size)
    )
    ...
    warehouse_invoices: List[
        WarehouseInvoice
    ] = warehouse_invoices_result.scalars().all()
    ...
    for warehouse_invoice in warehouse_invoices:
        response_updated_timestamp = max(
            response_updated_timestamp, warehouse_invoice.updated_at
        )
        response_updated_timestamp = max(
            response_updated_timestamp, warehouse_invoice.parent_invoice.updated_at
        )

    warehouse_invoices = list(
        filter(lambda x: x.status != InvoiceStatus.IN_TRANSIT, warehouse_invoices)
    )
    ...
    warehouse_inventories_result = await session.execute(
        select(
            WarehouseInventory.id, WarehouseInventory.row, WarehouseInventory.column
        ).where(
            WarehouseInventory.id.in_(warehouse_inventory_ids),
        )
    )
    warehouse_inventories = warehouse_inventories_result.all()
    warehouse_inventories_map = {
        warehouse_inventory[0]: warehouse_inventory
        for warehouse_inventory in warehouse_inventories
    }

    warehouse_invoices_response: dict = {
        "last_updated_at": response_updated_timestamp,
        "invoices": [],
    }
    for warehouse_invoice in warehouse_invoices:
        warehouse_invoice_response = WarehouseInvoiceSchema(
            ...
            warehouse_items=[
                WarehouseInventoryPick(
                    row=warehouse_inventories_map[
                        warehouse_invoice.warehouse_inventories[i]
                    ].row,
                    column=warehouse_inventories_map[
                        warehouse_invoice.warehouse_inventories[i]
                    ].column,
                ...
                for i in range(len(warehouse_invoice.skus))
                if warehouse_invoice.warehouse_inventories[i]
            ]
            + [
                WarehouseInventoryPick(
                    row=None,
                ...
                for i in range(len(warehouse_invoice.skus))
                if not warehouse_invoice.warehouse_inventories[i]
            ],
        )
...
async def post_warehouse_invoice_details(
    ...
    warehouse_inventories = await session.execute(
        select(WarehouseInventory).where(
            WarehouseInventory.id.in_(warehouse_invoice.warehouse_inventories)
        )
    )

    warehouse_inventories: List[
        WarehouseInventory
    ] = warehouse_inventories.scalars().all()
    ...
    sorted_time_per_item = sorted(
        [
            (index, time)
            for index, time in enumerate(warehouse_invoice_details.time_per_item)
        ],
        key=lambda x: x[1],
    )
    ...
    if not updates_are_matching:
        warehouse_invoice_details_object.columns = list(
            map(lambda x: x["column"], actual_updates)
        )
        warehouse_invoice_details_object.rows = list(
            map(lambda x: x["row"], actual_updates)
        )
        warehouse_invoice_details_object.quantities = list(
            map(lambda x: x["quantity"], actual_updates)
        )
        warehouse_invoice_details_object.sku_variants = list(
            map(lambda x: x["sku_variant"], actual_updates)
        )
